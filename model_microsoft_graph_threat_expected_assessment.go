/*
 * Partial Graph API
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * API version: v1.0
 * Generated by: OpenAPI Generator (https://openapi-generator.tech)
 */

package msgraph

// MicrosoftGraphThreatExpectedAssessment the model 'MicrosoftGraphThreatExpectedAssessment'
type MicrosoftGraphThreatExpectedAssessment string

// List of microsoft.graph.threatExpectedAssessment
const (
	MICROSOFTGRAPHTHREATEXPECTEDASSESSMENT_BLOCK   MicrosoftGraphThreatExpectedAssessment = "block"
	MICROSOFTGRAPHTHREATEXPECTEDASSESSMENT_UNBLOCK MicrosoftGraphThreatExpectedAssessment = "unblock"
)
